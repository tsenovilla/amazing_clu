...
#[test] 
// The execute function calls the other Grep's functions and propagates their errors. Let's test:
...
fn grep_execute_test(){
    ...
    let mut binary = PathBuf::from(manifest_dir); // We build the path to the binary in the debuf mode!
    binary.push("target");
    binary.push("debug");
    ...
    cmd.assert().stdout(contains("sample_text.txt")); // The output contains the name of the file and the matched pattern. It does not contain the hidden text
    cmd.assert().stdout(contains("3:I'm grep"));
    ...
    cmd.assert().stdout(contains("sample_text.txt"));
    cmd.assert().stdout(contains("2-How are you? Who are you?\n3:I'm grep\n4-Nice to meet you"));
    // .hidden_text.txt found!
    cmd.assert().stdout(contains(".hidden_text.txt"));
    cmd.assert().stdout(contains("1:I'm a hidden file created to test grep!"));
    // sample_text2.txt found
    cmd.assert().stdout(contains("sample_text2.txt"));
    cmd.assert().stdout(contains("1-I'm contained into a hidden folder.\n2:I'd like to test grep"));
